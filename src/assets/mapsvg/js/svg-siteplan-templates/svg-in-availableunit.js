jQuery(document).ready(function() {
    ...
    function mapSvgInit() {
        ...
        var SVGMap = JSON.parse(localStorage.getItem("SVGMap"));
        var cons_projectId = SVGMap.projectId;
        var cons_clusterId = SVGMap.clusterId;
        var cons_unitType = SVGMap.unitType;
        var cons_zoningID = SVGMap.zoningId;
        var cons_bedroom = SVGMap.bedroom;

        /* Start: Request API (Diagramatic/GetListDiagramaticWeb) */
        var startMapSVGInit = new Date();
        var startGETGetListDiagramaticSVGWeb = new Date();
        $.ajax({
            url: backendOrigin + 'api/services/app/Diagramatic/GetListDiagramaticSVGWeb',
            type: 'GET',
            dataType: 'json',
            ...
            data: "projectId=" + cons_projectId + "&clusterId=" + cons_clusterId + "&bedroom=" + cons_bedroom + "&unitType=" + cons_unitType + "&zoningID=" + cons_zoningID,
            success: function(data, textStatus, xhr) {
                console.log("ALL ", data.result);
                console.log("TIME - [2] GET GetListDiagramaticSVGWeb", daysBetween(startGETGetListDiagramaticSVGWeb, new Date()));
                ...
                console.log("Request GET - GetListDiagramaticSVGWeb : ", objDiagramatic);
                    ...
                    onClick: function(e, m) {
                        ...
                        if (!isEmpty(region.data) && region.isAvailable) {
                            if (isLoggedIn(tmpCookie)) {
                                ...
                                var ppNo = false;
                                var psCode = false;
                                var bookingUnitLink = "/app/main/booking-unit/" + projectId + "/" + clusterId + "/" + unitNo + "/" + unitCode + "/" + unitId + "/" + ppNo + "/" + psCode;
                        ...
                        if (!isEmpty(region.data)) {
                            ...
                            requestDetailUnit = $.ajax({
                                ...
                                success: function(data, textStatus, xhr) {
                                    ...
                                    if (localStorage.getItem("PPnHover") == 'false') {
                                    ...
                                    } if (localStorage.getItem("PPnHover") == 'true') {
                                        /* Start: Set Tooltip for Hovered Region */
                                        region.setTooltip("" +
                                            "<!DOCTYPE html>\n" +
                                            "<html class='tooltip custom'>\n" +
